...
numero_maximo_linhas = 71010

# Itera pelas linhas da planilha
for linha in range(2, numero_maximo_linhas + 2):  # Assumindo que os dados começam na linha 4
    ...
    pyautogui.press('tab')
    texto = str(sheet[f'S{linha}'].value) # VALOR DA BASE DE CÁLCULO (MULTIPLICAR POR CEM NO EXCEL)
    ...
    pyautogui.write(texto)
    pyautogui.press('tab')
    texto = str(sheet[f'T{linha}'].value)# VALOR DO DARF
    pyautogui.write(texto)
